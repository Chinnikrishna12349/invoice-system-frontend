...
const drawInvoiceContent = async (
    ...
    const effectiveCountry = invoice.country || 'india';
    ...
    for (const [index, service] of invoice.services.entries()) {
        ...
        if (yPosition + rowHeight > 265) {
            ...
            await addTextToPdf(doc, t.description, colX[1] + 2, textY, { fontSize: 10, align: 'left', language });
...
export const generateInvoicePDF = async (invoice: Invoice, language: 'en' | 'ja' = 'en', companyInfoParam?: CompanyInfo | null, silent: boolean = false) => {
    ...
    try {
        ...
        // Standardized File Naming across both Downloads and Email Attachments (includes description/resourceName)
        const langTag = language === 'ja' ? ' (Japanese)' : ' (English)';
        const baseName = resourceName ? `${invoice.invoiceNumber} ${resourceName}` : `${invoice.invoiceNumber}`;
        const fileName = `${baseName}${langTag}.pdf`.trim();
